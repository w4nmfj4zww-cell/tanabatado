...
const KarameJurokuwariTaskPage: React.FC = () => {
  const { level: levelString } = useParams<{ level: string }>();
  const level = levelString ? parseInt(levelString, 10) : 2; // Default to 2

  const problems = useMemo(() => Array.from({ length: level }, (_, i) => {
    const n = i + 1;
    // Perform division and handle floating point inaccuracies
    const result = n / level;
    // Format to a string, removing trailing zeros unless it's just '0'
    const ans = parseFloat(result.toFixed(10)).toString();
  ...
  const [timeLeft, setTimeLeft] = useState(60); // Longer time for division
    ...
    <div style={{ display: 'flex', flexDirection: 'column', height: '100vh', backgroundColor: '#212529', color: '#f8f9fa' }}>
      <div style={{ flex: 1, overflowY: 'auto', padding: '20px' }}>
        ...
        <div style={{ marginTop: '20px', textAlign: 'center' }}>
          <Link to="/karame-jurokuwari-menu" style={{ color: '#8ab4f8' }}>メニューに戻る</Link>
        </div>
      </div>
...
export default KarameJurokuwariTaskPage;
